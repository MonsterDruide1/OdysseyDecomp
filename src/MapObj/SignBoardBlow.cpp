...
SignBoardBlow::SignBoardBlow(const char* actorName, const char* signBoardBlowName)
    ...
    mName = signBoardBlowName;
}

void SignBoardBlow::init(const al::ActorInitInfo& info) {
    al::initActorWithArchiveName(this, info, mName, nullptr);
...
void SignBoardBlow::startBlow(const sead::Vector3f& trans) {
    appear();
    al::startHitReaction(this, "出現");
    al::setVelocityBlowAttack(this, trans, 18.0f, 38.0f);
    al::setNerve(this, &Blow);
    sead::Vector3f transDiff = al::getTrans(this) - trans;
    transDiff.y = 0.0f;
    mRotationAxis = sead::Vector3f::ey.cross(transDiff);
