...
void FishingUpObj::init(const al::ActorInitInfo& info) {
    const char* modelName = "Pukupuku";
    const char* archiveName = "FishingUpObj";
    al::tryGetStringArg(&modelName, info, "FishingUpObj");
    if (!al::isEqualString("Pukupuku", modelName))
        archiveName = nullptr;

    al::initActorWithArchiveName(this, info, modelName, archiveName);
    al::initNerve(this, &PopUp, 1);

    al::ByamlIter initFishingIter;
    if (al::tryGetActorInitFileIter(&initFishingIter, this, "InitFishing", archiveName))
        al::tryGetByamlString(&mItemGeneratePositionJointName, initFishingIter,
                              "ItemGeneratePositionJointName");

    if (al::isEqualString(modelName, "KinopioBrigade")) {
        mTalkNpcParam = rs::initTalkNpcParam(this, archiveName);
        mTalkNpcActionAnimInfo = new TalkNpcActionAnimInfo();
        mTalkNpcActionAnimInfo->setWaitActionName("Wait");
        mTalkNpcActionAnimInfo->init(this, info, mTalkNpcParam, archiveName);
        ...
        if (al::isExistLinkChild(info, "ShineActor", 0)) {
            ...
            s32 shineIndex = mLinkedShineIndex;
...
void FishingUpObj::appear() {
    ...
    if (al::isEqualString(al::getModelName(this), "KinopioBrigade")) {
        al::setScaleAll(this, 1.0f);
    } else {
        bool isPochi = al::isEqualString(al::getModelName(this), "Pochi");
        if (itemGenerator && !isPochi && itemGenerator->isShine())
...
bool FishingUpObj::receiveMsg(const al::SensorMsg* message, al::HitSensor* other,
    ...
    bool isInvalidTrample = rs::isInvalidTrampleSensor(self, mTalkNpcParam);
    if (isInvalidTrample) {
...
bool FishingUpObj::isAfterGiveShine() const {
    ...
    GameDataHolderAccessor accessor(this);
    s32 shineIndex = mLinkedShineIndex;
    return GameDataFunction::isGotShine(accessor, shineIndex);
}
...
bool FishingUpObj::tryUpdateShineHintTrans(const sead::Vector3f& trans) {
    if (mLinkedShineIndex >= 0) {
        const al::IUseSceneObjHolder* sceneObjHolder = this;
        GameDataHolderAccessor accessor(sceneObjHolder);
        s32 shineIndex = mLinkedShineIndex;
        if (!GameDataFunction::isGotShine(accessor, shineIndex)) {
            GameDataHolderAccessor accessor2(sceneObjHolder);
            s32 shineIndex2 = mLinkedShineIndex;
            GameDataFunction::setHintTrans(accessor2, shineIndex2, trans);
            return true;
        }
    }
