...
void AppearSwitchTimer::init(const al::ActorInitInfo& info, const al::IUseAudioKeeper* audioKeeper,
    ...
    if (!al::tryGetBoolArgOrFalse(info, "IsValidObjectCamera")) {
        ...
        s32 demoCameraFrame = -1;
        if (al::tryGetArg(&demoCameraFrame, info, "DemoCameraFrame") && demoCameraFrame != -1)
            stepPlaceholder = demoCameraFrame;
    }
...
void AppearSwitchTimer::exeOnWait() {
    ...
    if (area != nullptr) {
        al::LiveActor* player = al::tryFindNearestPlayerActor(mActor);
        if (player != nullptr) {
            if (!al::isInAreaPos(area, al::getTrans(player))) {
                offSwitch();
                return;
            }
        }
