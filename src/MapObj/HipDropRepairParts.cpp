...
void HipDropRepairParts::init(const al::ActorInitInfo& info) {
    ...
    mSubActor = al::tryGetSubActor(this, "跡モデル");
    if (mSubActor)
        mSubActor->makeActorDead();
...
void HipDropRepairParts::startHide() {
    ...
    if (mSubActor)
        al::hideModelIfShow(mSubActor);
}

void HipDropRepairParts::startShow() {
    al::showModelIfHide(this);
    if (mSubActor)
        al::showModelIfHide(mSubActor);
}
...
void HipDropRepairParts::exeOn() {
    ...
    sead::Vector3f trans = sead::Vector3f::ey * moveOffset + mInitTrans;
