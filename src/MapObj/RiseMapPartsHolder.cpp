...
// NON-MATCHING: https://decomp.me/scratch/lCb5J
void RiseMapPartsHolder::init(const al::ActorInitInfo& info) {
    ...
    mSameWorldSaveInfo = rs::createSaveObjInfoNoWriteSaveDataInSameWorld(info);
    ...
    if (al::isExistLinkChild(info, "PlayerRestartPos", 0)) {
        al::getLinksMatrix(&mMtx, info, "PlayerRestartPos");
        mIsRestartPos = true;
    }
...
void RiseMapPartsHolder::startRise() {
    ...
    if (!mIsSwitch)
        rs::onSaveObjInfo(mSameWorldSaveInfo);
