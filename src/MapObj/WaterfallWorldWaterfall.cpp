...
void WaterfallWorldWaterfall::exeChangeMaxToMin() {
    s32 maxStep = 10;
    al::IUseEffectKeeper* effectKeeper = this;
    f32 rate = al::calcNerveRate(this, maxStep);

    al::setEffectParticleAlpha(effectKeeper, "Wait", al::lerpValue(1.0f, 0.1f, rate));

    if (!al::isGreaterEqualStep(this, maxStep))
        return;
...
void WaterfallWorldWaterfall::exeChangeMinToMax() {
    ...
    al::setEffectParticleAlpha(effectKeeper, "Wait", al::lerpValue(0.1f, 1.0f, rate));
...
const sead::Matrix34f* WaterfallWorldWaterfall::getBaseMtx() const {
    return &mBaseMtx;
}
