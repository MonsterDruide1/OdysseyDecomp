...
FastenerKnob::FastenerKnob(const char* name)
    : al::LiveActor(name), _108(0), mIsUnknown110(false), mStartVertex(nullptr),
      mCurrentVertex(nullptr), mResetQuat(sead::Quatf::unit) {}
...
void FastenerKnob::exeWait() {
    ...
    if (mCurrentVertex->unk280 != 0) {
        if (mCurrentVertex->isUnk26b)
