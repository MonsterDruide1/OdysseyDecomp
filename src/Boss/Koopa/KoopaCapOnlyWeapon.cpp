...
NERVES_MAKE_NOSTRUCT(KoopaCapOnlyWeapon, WaitHoverDelay, WaitHoverStart, WaitHover)
}  // namespace
...
void KoopaCapOnlyWeapon::startWaitHover(s32 delay) {
    ...
    if (delay >= 1) {
        nerve = &WaitHoverDelay;
        mWaitHoverDelay = delay;
    } else {
        nerve = &WaitHoverStart;
    }
...
bool KoopaCapOnlyWeapon::isEndWaitHoverStart() const {
    return !al::isNerve(this, &WaitHoverDelay) && !al::isNerve(this, &WaitHoverStart);
}

bool KoopaCapOnlyWeapon::isWaitHover() const {
    return al::isNerve(this, &WaitHoverDelay) || al::isNerve(this, &WaitHoverStart) ||
           al::isNerve(this, &WaitHover);
}

void KoopaCapOnlyWeapon::attackSensor(al::HitSensor* self, al::HitSensor* other) {
    if (al::isSensorEnemyAttack(self)) {
        if (al::isNerve(this, &WaitHoverDelay) || al::isNerve(this, &WaitHoverStart) ||
            al::isNerve(this, &WaitHover)) {
...
void KoopaCapOnlyWeapon::exeWaitHoverDelay() {
    al::setNerveAtGreaterEqualStep(this, &WaitHoverStart, mWaitHoverDelay);
}
...
void KoopaCapOnlyWeapon::exeWaitHoverStart() {
    ...
    al::setNerveAtActionEnd(this, &WaitHover);
}
