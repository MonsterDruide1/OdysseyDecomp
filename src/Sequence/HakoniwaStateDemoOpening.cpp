...
SceneThreadInfo sSceneThreadInfo __attribute__((used)) = {
...
HakoniwaStateDemoOpening::HakoniwaStateDemoOpening(
    ...
    : al::HostStateBase<HakoniwaSequence>("オープニングデモ", sequence),
      mFirstSequenceScene(nullptr), mDemoScene(nullptr), mWipeHolder(wipeHolder),
      mBootLayout(bootLayout), mStartOpeningDemo(nullptr),
      mScreenCaptureExecutor(screenCaptureExecutor), mResourceLoader(resourceLoader),
      mIsSecond(false), mStateDeleteScene(stateDeleteScene), mInitThread(initThread),
...
void HakoniwaStateDemoOpening::exeLoadFirstSequence() {
    ...
    if (!mInitThread->isDone())
        return;
    if (!mIsSecond && al::isLessEqualStep(this, 540))
        return;
...
void HakoniwaStateDemoOpening::exeFadeToDemo() {
    if (!al::isDead(mStartOpeningDemo))
        return;
    if (!al::tryEndSceneInitThread(getHost()))
        return;
