...
StageSceneLayout::StageSceneLayout(const char* name, const al::LayoutInitInfo& initInfo,
    ...
    : al::NerveStateBase(name), mCoinCounter(nullptr), mCounterLifeCtrl(nullptr),
      mShineCounter(nullptr), mCoinCollectCounter(nullptr), mShineChipLayoutParts(nullptr),
      mPlayGuideCamera(nullptr), mPlayGuideBgm(nullptr), mMapMini(nullptr),
      mPlayerHolder(playerHolder), mIsOnlyCoin(false), mPlayGuideMenu(nullptr),
      mGamePadPlayStyleInfo(nullptr), mLayoutActorKidsMode(nullptr) {
    initNerve(&End, 0);
    ...
    if (rs::isExistShineChipWatcher(mShineChipLayoutParts) &&
        ...
        al::startAction(mShineChipLayoutParts, "Wait");
    }
...
void StageSceneLayout::start() {
    ...
    GameDataHolderAccessor accessor(mCoinCollectCounter);
    if (GameDataFunction::getCoinCollectNumMax(accessor) != 0)
        mCoinCollectCounter->tryStart();
    else
        al::killLayoutIfActive(mCoinCollectCounter);

    if (rs::isExistShineChipWatcher(mShineChipLayoutParts)) {
        if (rs::getShineChipCount(mShineChipLayoutParts) >= 1 &&
...
bool StageSceneLayout::tryStartLifeDemo() {
    CoinCounter* coinCounter = mCoinCounter;
    CounterLifeCtrl* counterLifeCtrl = mCounterLifeCtrl;
    GameDataHolderAccessor accessor(coinCounter);
    return counterLifeCtrl->tryUpdateCount(GameDataFunction::getPlayerHitPointMaxCurrent(accessor));
}
...
bool StageSceneLayout::isEndCoinCountAnim() const {
    ...
    if (al::isDead(mCoinCollectCounter))
        return true;

    return mCoinCollectCounter->isWait();
}
...
void StageSceneLayout::exeAppear() {
    CoinCounter* coinCounter = mCoinCounter;
    if (coinCounter->isAlive()) {
        const al::IUseLayoutAction& layoutAction = *coinCounter;
...
void StageSceneLayout::exeEnd() {
    if (al::isFirstStep(this)) {
        if (rs::isExistShineChipWatcher(mShineChipLayoutParts) &&
            !rs::isAppearedShineChipShine(mShineChipLayoutParts)) {
            al::startAction(mShineChipLayoutParts, "End");
        }
    ...
    const CoinCounter& coinCounter = *mCoinCounter;
    if (coinCounter.isAlive()) {
        const al::IUseLayoutAction& layoutAction = coinCounter;
        if (al::isActionEnd(&layoutAction))
            al::killLayoutIfActive(mShineChipLayoutParts);
