...
void ShopUtil::makeExplainLabel(al::StringTmp<256>* result, const char* firstString,
                                const char* secondString, bool isAddPerson) {
    const char* person = isAddPerson ? "Woman" : "";
    result->format("%s%s%s", firstString, secondString, person);
}
