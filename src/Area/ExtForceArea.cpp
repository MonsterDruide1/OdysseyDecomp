...
void ExtForceArea::calcExtForce(sead::Vector3f* force, const sead::Vector3f& pos,
    ...
    al::separateVectorHV(&horizPrevVel, &vertPrevVel, dirUp, velocity);

    sead::Vector3f horizVel = {0.0f, 0.0f, 0.0f};
    sead::Vector3f vertVel = {0.0f, 0.0f, 0.0f};
    al::separateVectorHV(&horizVel, &vertVel, dirUp, prevForce);
    ...
    if (al::isSameSign(prevVertSpeed, mSpeedTarget) &&
        sead::Mathf::abs(prevVertSpeed) > sead::Mathf::abs(mSpeedTarget)) {
