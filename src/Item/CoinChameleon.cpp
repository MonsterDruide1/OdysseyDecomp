...
void CoinChameleon::init(const al::ActorInitInfo& initInfo) {
    ...
    al::tryGetDisplayOffset(&mTrans, initInfo);

    mAppearRotate = new CoinStateAppearRotate(this, mMtxConnector, mTrans, "出現");
    al::initNerveState(this, mAppearRotate, &NrvCoinChameleon.Appear, "出現");
...
bool CoinChameleon::receiveMsg(const al::SensorMsg* message, al::HitSensor* other,
    ...
    if (!rs::isVisibleChameleon(message))
        return false;
    if (!al::isSensorName(self, "Hipdrop"))
        return false;
...
void CoinChameleon::exeVisible() {
    ...
    al::setModelMaterialParameterF32(this, "CoinW6Mat00", "const_single0",
                                     al::getNerveStep(this) / -120.0f + 1.0f);
...
void CoinChameleon::exeAppear() {
    if (al::updateNerveState(this)) {
        mCoin->appearCoinChameleon(al::getTrans(this), al::getTrans(this), mTrans);
        kill();
        return;
    }
