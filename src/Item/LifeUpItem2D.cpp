...
NERVES_MAKE_NOSTRUCT(LifeUpItem2D, Wait, AutoGetDemo, PopUp);
...
void appearIn2DArea(LifeUpItem2D* actor, ActorDimensionKeeper* dimensionKeeper) {
    ...
    const IUseDimension* dimension = actor;
    if (!rs::isIn2DArea(dimension))
        return;

    al::onCollide(actor);
    rs::snap2DParallelizeFront(actor, dimension, 500.0f);
    sead::Vector3f velocity = al::getGravity(actor) * -11.0f;
...
ActorDimensionKeeper* LifeUpItem2D::getActorDimensionKeeper() const {
