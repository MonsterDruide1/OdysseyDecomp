...
ShineChipWatcherHolder::ShineChipWatcherHolder() {
    ...
    mWatchers.allocBuffer(8, nullptr, 8);
}
...
ShineChipWatcher* ShineChipWatcherHolder::getCurrentWatcher() const {
    s32 currentWatcherIndex = mCurrentWatcherIndex;
    u32 watcherCount = mWatchers.size();
    u32 currentWatcherIndexU = currentWatcherIndex;

    if (watcherCount <= currentWatcherIndexU)
        return nullptr;

    return mWatchers[currentWatcherIndex];
}

bool ShineChipWatcherHolder::tryUpdateCurrentWatcher(const al::LiveActor* actor) {
    s32 watcherCountMax = mWatchers.size();
    if (watcherCountMax < 1)
        return false;

    for (s32 i = 0; i < watcherCountMax; i++) {
        u32 currentCount = mWatchers.size();
        ShineChipWatcher* watcher = currentCount <= u32(i) ? nullptr : mWatchers[i];
...
bool isCompleteShineChip(const al::IUseSceneObjHolder* objHolder) {
    s32 completeNum = ShineChipWatcher::getCompleteNum();
