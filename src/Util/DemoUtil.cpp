...
bool requestStartDemoActor(al::LiveActor* actor, const char* demoName,
...
bool requestStartDemoActorWithSubActor(al::LiveActor* actor, bool isSubActor, const char* demoName,
                                       ProjectDemoDirector::DemoType demoType) {
    ProjectDemoDirector* director = getDirector(actor);
    if (!director->requestStartDemo(demoName, demoType))
        return false;

    ProjectDemoDirector* addDirector = getDirector(actor);
    if (isSubActor)
        addDirector->addDemoActorWithSubActor(actor);
    else
        addDirector->addDemoActor(actor);
    getDirector(actor)->setRequestActor(actor);
    return true;
}

bool requestStartDemoScene(const al::Scene* scene, const char* demoName,
                           ProjectDemoDirector::DemoType demoType) {
    return getDirector(scene)->requestStartDemo(demoName, demoType);
}

void requestEndDemoActor(const al::LiveActor* actor, const char* demoName,
                         ProjectDemoDirector::DemoType demoType) {
    getDirector(actor)->requestEndDemo(demoName, demoType);
}

void requestEndDemoScene(const al::Scene* scene, const char* demoName,
                         ProjectDemoDirector::DemoType demoType) {
    getDirector(scene)->requestEndDemo(demoName, demoType);
}

bool isActiveDemoScene(const al::Scene* scene, const char* demoName) {
    return getDirector(scene)->isActiveDemo(demoName);
}

bool isActiveDemoActor(const al::LiveActor* actor, const char* demoName) {
    return getDirector(actor)->isActiveDemo(demoName);
}

bool isRandomActionDemoChangeWorldName(const char* actionName) {
    if (actionName == nullptr)
        return false;

    if (al::isEqualString(actionName, cActionDemoWorldMoveForwardNormal))
        return true;
    if (al::isEqualString(actionName, cActionDemoWorldMoveForward))
        return true;
    return al::isEqualString(actionName, cActionDemoWorldMoveBackward);
}
}  // namespace
...
bool isRandomActionDemoChangeWorld(const al::LiveActor*, const char* actionName) {
    return isRandomActionDemoChangeWorldName(actionName);
}
...
bool tryGetDemoChangeWorldRandomActionName(sead::BufferedSafeString* actionNameOut, s32* indexOut,
    ...
    if (isRandomActionDemoChangeWorldName(actionName) &&
        GameDataFunction::isGameClear(GameDataHolderAccessor(actor))) {
        s32 index = getRandomActionIndexDemoChangeWorld(actor);
        *indexOut = index;
        if (index != 0)
            actionNameOut->format("%s%d", actionName, index);
        else
...
bool requestStartDemoNormal(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoNormal, demoTypeNormal());
}
...
bool requestStartDemoNormalWithCinemaFrame(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoNormalWithCinemaFrame,
...
bool requestStartDemoWithPlayer(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoWithPlayer,
                                             demoTypeWithPlayer());
}
...
bool requestStartDemoWithPlayerCinemaFrame(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoWithPlayerCinemaFrame,
...
bool requestStartDemoWithPlayerKeepCarry(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoWithPlayerKeepCarry,
...
bool requestStartDemoWithPlayerUseCoin(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoWithPlayerUseCoin,
                                             demoTypeWithPlayerKeepCarry());
}
...
bool requestStartDemoWithPlayerKeepBindTalk(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoWithPlayerKeepBindTalk,
...
bool requestStartDemoKeepHackTalk(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoKeepHackTalk,
...
bool requestStartDemoWithPlayerCinemaFrameTalk(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoWithPlayerCinemaFrameTalk,
                                             demoTypeWithPlayerCinemaFrame());
}
...
bool requestStartDemoWithPlayerKeepCarryTalk(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoWithPlayerKeepCarryTalk,
                                             demoTypeWithPlayerKeepCarry());
}
...
bool requestStartDemoWithPlayerUseCoinTalk(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoWithPlayerUseCoinTalk,
                                             demoTypeWithPlayerKeepCarry());
}
...
bool requestStartDemoNormalTalk(al::LiveActor* actor, bool isSubActor) {
    return requestStartDemoActorWithSubActor(actor, isSubActor, cDemoNormalTalk, demoTypeNormal());
}
...
bool requestStartDemoBossBattleStart(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoWithPlayerCinemaFrame,
                                 demoTypeWithPlayerCinemaFrame());
}
...
bool requestStartDemoShineAppear(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoShineAppear, demoTypeNormal());
}
...
bool requestStartDemoWarp(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoWarp, demoTypeWithPlayerKeepCarry());
}
...
bool requestStartDemoHackStart(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoHackStart, demoTypeWithPlayer());
}
...
bool requestStartDemoAppearByBazookaElectric(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoAppearByBazookaElectric, demoTypeWithPlayer());
}
...
bool requestStartDemoLaunchBazookaElectric(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoLaunchBazookaElectric, demoTypeNormal());
}
...
bool requestStartDemoSceneStartPlayerWalk(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoSceneStartPlayerWalk, demoTypeSceneStartPlayerWalk());
}
...
bool requestStartDemoBirdCarryMeat(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoBirdCarryMeat, demoTypeNormalWithCinemaFrame());
}
...
bool requestStartDemoBirdFindMeat(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoBirdFindMeat, demoTypeNormal());
}
...
bool requestStartDemoGetTimeBalloon(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoGetTimeBalloon, demoTypeWithPlayer());
}
...
bool requestStartDemoJango(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoJango, demoTypeWithPlayer());
}
...
bool requestStartDemoGoToEnding(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoGoToEnding, demoTypeNormal());
}
...
bool isActiveDemoWithPlayer(const al::Scene* scene) {
    if (getDirector(scene)->isActiveDemoWithPlayer())
        return true;
    return getDirector(scene)->isActiveDemo(cDemoKeepHackTalk);
}

bool isActiveDemoWithPlayer(const al::LiveActor* actor) {
    if (getDirector(actor)->isActiveDemoWithPlayer())
        return true;
    return getDirector(actor)->isActiveDemo(cDemoKeepHackTalk);
}

bool isActiveDemoWithPlayerCinemaFrame(const al::Scene* scene) {
    if (isActiveDemoScene(scene, cDemoWithPlayerCinemaFrame))
        return true;
...
bool isActiveDemoWithPlayerCinemaFrame(const al::LiveActor* actor) {
    if (isActiveDemoActor(actor, cDemoWithPlayerCinemaFrame))
        return true;
    return isActiveDemoActor(actor, cDemoWithPlayerCinemaFrameTalk);
}

bool isActiveDemoWithPlayerKeepCarry(const al::Scene* scene) {
    if (isActiveDemoScene(scene, cDemoWithPlayerKeepCarry))
        return true;
...
bool isActiveDemoWithPlayerKeepCarry(const al::LiveActor* actor) {
    if (isActiveDemoActor(actor, cDemoWithPlayerKeepCarry))
        return true;
    return isActiveDemoActor(actor, cDemoWithPlayerKeepCarryTalk);
}

bool isActiveDemoWithPlayerUseCoin(const al::Scene* scene) {
    if (isActiveDemoScene(scene, cDemoWithPlayerUseCoin))
        return true;
...
bool isActiveDemoWithPlayerUseCoin(const al::LiveActor* actor) {
    if (isActiveDemoActor(actor, cDemoWithPlayerUseCoin))
        return true;
    return isActiveDemoActor(actor, cDemoWithPlayerUseCoinTalk);
}
...
bool isActiveDemoHackStartFirst(const al::Scene* scene) {
    const char* demoName = getDirector(scene)->getDemoInfoDemoName();
    if (demoName != nullptr) {
        if (al::isEqualString(demoName, DemoName::cHackStartFirstTimeFrog))
            return true;

        demoName = getDirector(scene)->getDemoInfoDemoName();
        if (demoName != nullptr)
            return al::isEqualString(demoName, DemoName::cHackStartFirstTimeKoopa);
    }
    return false;
}
...
bool isEqualDemoInfoDemoName(const al::Scene* scene, const char* demoName) {
    ...
    if (activeDemoName != nullptr)
        return al::isEqualString(activeDemoName, demoName);
    return false;
}
...
bool isActiveDemoTalk(const al::Scene* scene) {
    if (isActiveDemoScene(scene, cDemoWithPlayerCinemaFrameTalk))
        return true;
    if (isActiveDemoScene(scene, cDemoWithPlayerKeepCarryTalk))
        return true;
    if (isActiveDemoScene(scene, cDemoKeepHackTalk))
        return true;
    if (isActiveDemoScene(scene, cDemoNormalTalk))
        return true;
    return isActiveDemoScene(scene, cDemoWithPlayerKeepBindTalk);
}
...
void requestEndDemoWorldIntroCamera(const al::Scene* scene) {
    ...
    const al::IUseSceneObjHolder* holder = scene;
    GameDataHolderAccessor skipAccessor(holder);
    SequenceDemoSkipData* skipData = skipAccessor->getGameDataFile()->getSequenceDemoSkipData();
    GameDataHolderAccessor worldAccessor(holder);
    skipData->showWorldIntroCamera(GameDataFunction::getCurrentWorldIdNoDevelop(worldAccessor));
}
...
void requestEndDemoScenarioCamera(const al::LiveActor* actor) {
    ...
    SequenceDemoSkipData* skipData = skipAccessor->getGameDataFile()->getSequenceDemoSkipData();
    GameDataHolderAccessor worldAccessor(holder);
    s32 worldId = GameDataFunction::getCurrentWorldIdNoDevelop(worldAccessor);
    skipData->showScenarioStartCamera(worldId, getActiveQuestNo(holder));
}
...
bool requestStartDemoPeachCastleCap(al::LiveActor* actor) {
    return requestStartDemoActor(actor, cDemoPeachCastleCap, demoTypeWithPlayer());
}
...
bool isFirstDemoWorldIntroCamera(const al::Scene* scene) {
    ...
    SequenceDemoSkipData* skipData = skipAccessor->getGameDataFile()->getSequenceDemoSkipData();
    GameDataHolderAccessor worldAccessor(holder);
    return !skipData->isAlreadyShowWorldIntroCamera(
        GameDataFunction::getCurrentWorldIdNoDevelop(worldAccessor));
}
...
bool isFirstDemoScenarioStartCamera(const al::LiveActor* actor) {
    ...
    GameDataHolderAccessor worldAccessor(holder);
    s32 worldId = GameDataFunction::getCurrentWorldIdNoDevelop(worldAccessor);
    return !skipData->isAlreadyShowScenarioStartCamera(worldId, getActiveQuestNo(holder));
}
