...
bool requestStartDemoBossBattleStart(al::LiveActor* actor) {
    return requestStartDemoWithPlayerCinemaFrame(actor, false);
}

void requestEndDemoBossBattleStart(const al::LiveActor* actor) {
    requestEndDemoWithPlayerCinemaFrame(actor);
}
...
bool isActiveDemoWithPlayer(const al::Scene* scene) {
    return getDirector(scene)->isActiveDemoWithPlayer() || isActiveDemoKeepHackTalk(scene);
}

bool isActiveDemoWithPlayer(const al::LiveActor* actor) {
    return getDirector(actor)->isActiveDemoWithPlayer() ||
           isActiveDemoActor(actor, "キャプチャ継続デモ[会話]");
}
...
bool isActiveDemoShineGet(const al::Scene* scene) {
    return isActiveDemoScene(scene, "シャイン取得デモ") || isActiveDemoShineDotGet(scene);
}

bool isActiveDemoShineGet(const al::LiveActor* actor) {
    return isActiveDemoActor(actor, "シャイン取得デモ") || isActiveDemoShineDotGet(actor);
}
...
bool isActiveDemoPlayerPuppetable(const al::LiveActor* actor) {
    return isActiveDemoWithPlayer(actor) || isActiveDemoWithPlayerCinemaFrame(actor) ||
           isActiveDemoWithPlayerKeepCarry(actor) || isActiveDemoWithPlayerUseCoin(actor) ||
           isActiveDemoWithPlayerKeepBindTalk(actor) || isActiveDemoShineGet(actor) ||
           isActiveDemoShineGrandGet(actor) || isActiveDemoShineMainGet(actor) ||
           isActiveDemoWarp(actor) || isActiveDemoHackStart(actor) ||
           isActiveDemoActor(actor, "ホームから出現するデモ") ||
           isActiveDemoActor(actor, "ホームに戻るデモ");
}
...
void addDemoSubActor(al::LiveActor* actor) {
    ...
    for (s32 i = 0; i < subActorNum; i++)
        addDemoActor(al::getSubActor(actor, i), true);
}
