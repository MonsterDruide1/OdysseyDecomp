...
void endHackAirVelocity(IUsePlayerHack** playerHack, const sead::Vector3f& targetPos,
                        const sead::Quatf& quat, const sead::Vector3f& vel, s32 frameDelay) {
    PlayerHackKeeper* hackKeeper = (*playerHack)->getPlayerHackKeeper();
    HackEndParam hackEndParam;
...
inline s32 getSegment(f32 target, f32 currentSegment, bool lookForward) {
    f32 prevStep = 0.0f;
    for (s32 i = 1; i < 600; i++) {
        f32 next = prevStep + currentSegment;
        if (al::isInRange(target, currentSegment, next))
            return i;

        prevStep = sead::Mathf::clamp(prevStep + ((lookForward) ? 1.0f : -1.0f), -35.0f, 35.0f);
        currentSegment = next;
    }
    return 600;
}
...
void endHackBazookaElectricToDestination(IUsePlayerHack** playerHack, const sead::Vector3f& start,
    ...
    if (startCpy.y < end.y)
        maxHeight = (end.y - startCpy.y) + height;
    path.initFromUpVector(startCpy, end, sead::Vector3f::ey, maxHeight);

    s32 step = getSegment(maxHeight, 0.0f, true);
    s32 step2 = getSegment(end.y, maxHeight + startCpy.y + 0.0f, false);

    maxHeight = 1.0f / (step2 + step);
    f32 vely = step;
    hackEndParam.vel = maxHeight * (end - startCpy);
