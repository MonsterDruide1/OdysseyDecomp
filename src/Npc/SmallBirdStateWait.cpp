...
void SmallBirdStateWait::exeTurn() {
    if (al::isFirstStep(this)) {
        ...
        sead::Vector3f target_dir = {0, 0, 0};
        if (mTurnRange >= 0) {
            al::rotateVectorDegree(&target_dir, mInitialFront, -al::getGravity(mActor),
                                   al::getRandom(sead::Mathf::min(15, mTurnRange), mTurnRange) *
                                       (al::isHalfProbability() ? -1 : 1));
        } else {
