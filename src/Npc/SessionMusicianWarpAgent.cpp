...
void registerPlayerRestartPosNormal(const al::PlacementInfo& info, al::LiveActor* actor) {
...
void registerPlayerRestartPosTalk(const al::PlacementInfo& info, al::LiveActor* actor) {
    al::PlacementInfo restartInfo = info;
...
ChangeStageInfo* createChangeStageNormal(const al::PlacementInfo& info, al::LiveActor* actor) {
...
ChangeStageInfo* createChangeStageTalk(const al::PlacementInfo& info, al::LiveActor* actor) {
    const char* stageName = nullptr;
    if (!al::tryGetStringArg(&stageName, info, "ChangeStageName"))
        return nullptr;
...
bool SessionMusicianWarpAgent::tryGetWarpTargetInfo(al::PlacementInfo* targetInfo) {
    const al::PlacementInfo& placement = *mPlacementInfo;

    const char* posName;
    if (GameDataFunction::getSessionEventProgress(mActor) > SessionEventProgress::Wait4thMusician)
        posName = "PlayerRestartPosTalk";
    else
        posName = "PlayerRestartPosNormal";

    return al::tryGetLinksInfo(targetInfo, placement, posName);
}
...
bool SessionMusicianWarpAgent::tryStartWarp() {
    if (mChangeStageNormal == nullptr && mChangeStageTalk == nullptr)
