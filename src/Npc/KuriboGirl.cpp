...
void KuriboGirl::attackSensor(al::HitSensor* source, al::HitSensor* target) {
    if (al::isSensorName(source, "WatchStart")) {
        if (rs::sendMsgKuriboGirlLove(target, source) && isWatchStart(al::getSensorPos(target))) {
            _111 = true;
            _113 = false;
        ...
        if (al::isSensorPlayerAll(target) && !rs::isPlayerHackKuriboAny(this) &&
            isWatchStart(al::getSensorPos(target))) {
            _111 = true;
            _113 = false;
    ...
    if (!al::isSensorName(source, "WatchFar") || !rs::sendMsgKuriboGirlLove(target, source)) {
        if (al::isSensorEnemyBody(source)) {
            al::sendMsgPush(target, source);
            rs::sendMsgPushToPlayer(target, source);
            return;
        }
        if (!al::isSensorEnemyAttack(source))
            return;
        if (!isNrvWait() && !al::isNerve(this, &NrvKuriboGirl.Appear))
            return;
        if (!rs::sendMsgKuriboGirlAttack(target, source))
            return;
