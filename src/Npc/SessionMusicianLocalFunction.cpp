...
bool isMusicianType(const al::LiveActor* actor, SessionMusicianType type) {
    return (u32)getMusicianType(actor) == type;
}
...
s32 getMemberMusicianNum(const al::LiveActor* actor) {
    SessionMusicianManager* manager = getSessionMusicianManager(actor);

    if (manager->isSubscribed(SessionMusicianType::Vocal))
        return SessionMusicianType::size();

    s32 count = 0;

    for (s32 i = 0; i < SessionMusicianType::size(); ++i)
        if (getSessionMusicianManager(actor)->isSubscribed(i))
            count++;
