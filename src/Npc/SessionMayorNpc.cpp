...
// TODO: add a function in sead for this?
...
static inline void startEventFlow(al::EventFlowExecutor* eventFlow, SessionMayorNpc* actor) {
    rs::startEventFlow(eventFlow, GameDataFunction::getSessionEventProgress(actor).text());
}
...
void SessionMayorNpc::init(const al::ActorInitInfo& initInfo) {
    ...
    // NOTE: only needs 1 slot, but reserves 8
    ...
    rs::initEventCameraTalk(mEventFlowExecutor, initInfo, "Outdoor", -1.0f);
    ...
    if (progress > SessionEventProgress::Wait1stMusician)
        al::onStageSwitch(this, "Join1stMusicianSwitchOn");

    if (progress > SessionEventProgress::RequestGoToThePowerPlant) {
        al::tryOnStageSwitch(this, "Join4thMusicianSwitchOn");
        if (progress < SessionEventProgress::TheCeremonyIsReady)
    ...
    // NOTE: 1 would suffice, but reserves 8 here
    ...
    mRandomWaitActionUpdater->setRandomOutbreakProbability(0.2f);
...
bool SessionMayorNpc::receiveMsg(const al::SensorMsg* message, al::HitSensor* other,
    ...
    if (!al::isNerve(this, &NrvHostType.Reaction) || !al::isNewNerve(this)) {
        ...
        if (rs::isInvalidTrampleSensor(self, mTalkNpcParam))
            isReaction = mNpcStateReaction->receiveMsgWithoutTrample(message, other, self);
        else
            isReaction = mNpcStateReaction->receiveMsg(message, other, self);
...
void SessionMayorNpc::exeReaction() {
    ...
    if (al::updateNerveStateAndNextNerve(this, &NrvHostType.Wait)) {
        al::startAction(this, GameDataFunction::getSessionEventProgress(this) >
