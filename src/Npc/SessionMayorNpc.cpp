...
ALWAYS_INLINE SessionEventProgress getProgress(const char* eventName) {
...
void SessionMayorNpc::init(const al::ActorInitInfo& initInfo) {
    ...
    {
        al::EventFlowExecutor* eventFlow = mEventFlowExecutor;
        GameDataHolderAccessor eventAccessor(this);
        rs::startEventFlow(eventFlow,
                           GameDataFunction::getSessionEventProgress(eventAccessor).text());
    }
...
ALWAYS_INLINE bool isCameraMusicianType(SessionMusicianType type) {
    return type >= SessionMusicianType::Drum && type <= SessionMusicianType::Trumpet;
}

ALWAYS_INLINE const char* getCameraName(SessionMusicianType type) {
    if (!isCameraMusicianType(type))
        return nullptr;
    return sCameraNames[type];
}

ALWAYS_INLINE sead::Vector3f* getCameraOffset(SessionMusicianType type, SessionMayorParam* param) {
    if (!isCameraMusicianType(type))
        return nullptr;
...
ALWAYS_INLINE f32 getCameraDistance(SessionMusicianType type, SessionMayorParam* param) {
    if (!isCameraMusicianType(type))
        return -1.0f;
    f32 values[] = {
...
ALWAYS_INLINE f32 getCameraHAngle(SessionMusicianType type, SessionMayorParam* param) {
    if (!isCameraMusicianType(type))
        return 45.0f;
...
ALWAYS_INLINE f32 getCameraVAngle(SessionMusicianType type, SessionMayorParam* param) {
    if (!isCameraMusicianType(type))
        return 10.0f;
...
void SessionMayorNpc::initIntroductionCamera(const al::ActorInitInfo& initInfo,
    ...
    for (auto it = musicians->begin(); it != musicians->end(); ++it) {
        ...
        if (!rs::isDefinedEventCamera(evenFlow, cameraName))
            continue;
        sead::Vector3f* offset = getCameraOffset(type, mayorParam);
        f32 distance = getCameraDistance(type, mayorParam);
        f32 hAngle = getCameraHAngle(type, mayorParam);
        f32 vAngle = getCameraVAngle(type, mayorParam);
