...
void SmallBirdStateFlyAway::exeFlyAway() {
    if (al::isFirstStep(this)) {
        ...
        static s32 gIndices[] = {0, 0, 0};

        mIsColliding = false;
        startActionAtRandomFrameIfNotPlaying(mActor, "Fly");
        if (rs::isModeE3MovieRom()) {
            mVerticalAccel = gE3MovieVerticalAccel[gIndices[0] = al::modi(gIndices[0] + 7, 6)];
            mHorizontalAccel = gE3MovieHorizontalAccel[gIndices[1] = al::modi(gIndices[1] + 5, 4)];
        } else {
            mVerticalAccel = gVerticalAccel[gIndices[0] = al::modi(gIndices[0] + 5, 4)];
            mHorizontalAccel = gHorizontalAccel;
        }
        mTargetAccelDir = {0, mVerticalAccel, mHorizontalAccel};
        al::normalize(&mTargetAccelDir);
        mCollisionCheckOffsetStep = gIndices[2] = al::modi(gIndices[2] + 11, 10);
    }
    ...
    if (al::isFirstStep(this) || al::isIntervalStep(this, 10, mCollisionCheckOffsetStep)) {
        ...
        if (mIsColliding) {
            ...
            if (distance < max_speed * 10 && !al::isFirstStep(this) &&
                !tryFaceToHorizontalDirection(mActor, triangle.getNormal(0)))
                mHorizontalAccel = 0;
        }
    }
