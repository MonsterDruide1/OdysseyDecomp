...
NERVES_MAKE_NOSTRUCT(FirstSequenceScene, Wipe)
NERVES_MAKE_NOSTRUCT(FirstSequenceScene, SelectMode)
}  // namespace
...
void FirstSequenceScene::init(const al::SceneInitInfo& initInfo) {
    ...
    al::WaveVibrationHolder* waveVibrationHolder = initInfo.gameSystemInfo->waveVibrationHolder;
    mWaveVibrationHolder = waveVibrationHolder;

    mStateSelectMode = new SceneStateSelectMode("モード選択", this, layoutInitInfo, nullptr,
                                                nullptr, waveVibrationHolder);
...
void FirstSequenceScene::drawMain() const {
    ...
    framebuffer->clear(drawContext, 3, sead::Color4f::cGray, al::getDepthClearValue(), 0);
    al::setRenderBuffer(getLayoutKit(), framebuffer);
