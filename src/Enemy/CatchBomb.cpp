...
void CatchBomb::init(const al::ActorInitInfo& info) {
    ...
    mLoopAnimState = new LoopAnimState(this, "Catch", false);
    al::initNerveState(this, mLoopAnimState, &NrvCatchBomb.Catch, "キャッチ");
    al::invalidateHitSensor(this, "Explosion");
    al::tryGetArg(&mIsRevival, info, "Revival");
    mPosition.set(al::getTrans(this));
...
inline bool isAppearOld(const CatchBomb* self) {
...
void CatchBomb::attackSensor(al::HitSensor* self, al::HitSensor* other) {
    ...
    if ((al::isSensorPlayer(other) || al::isSensorEnemyBody(other)) &&
        ...
        // There's an inline of isEnableEnemyAttack here, but that function doesn't match.
        if (isAppearOld(this) || al::isNerve(this, &NrvCatchBomb.Explode) || isEnableExplosion() ||
...
// NON-MATCHING: https://decomp.me/scratch/YFClJ
bool CatchBomb::isEnableEnemyAttack() const {
    return isAppearOld(this) || al::isNerve(this, &NrvCatchBomb.Explode) || isEnableExplosion();
}
...
bool CatchBomb::receiveMsg(const al::SensorMsg* message, al::HitSensor* other,
    ...
    if (al::isNerve(this, &NrvCatchBomb.Wait) && al::isSensorName(self, "Body") &&
    ...
    if (!isUncapNerve()) {
        if (rs::isMsgCapStartLockOn(message)) {
            mSensorHost = al::getSensorHost(other);
    ...
    return rs::isMsgNpcScareByEnemy(message);
}
...
bool CatchBomb::isUncapNerve() const {
    return isAppearOld(this) || al::isNerve(this, &NrvCatchBomb.Back) ||
           al::isNerve(this, &NrvCatchBomb.Hide) || al::isNerve(this, &NrvCatchBomb.Catch) ||
           isEnableExplosion() || al::isNerve(this, &NrvCatchBomb.Explode) ||
           al::isNerve(this, &NrvCatchBomb.Throw);
}
...
void CatchBomb::exeWait() {
    ...
    if (_148 == 0) {
        if (al::isNear(this, al::getPlayerActor(this, 0), 1000.0f)) {
            al::setNerve(this, &NrvCatchBomb.Find);
            return;
        }
    }

    if (_148 > 120 && al::isFar(this, mPosition, 300.0f))
...
bool CatchBomb::checkCollisionExplode(const sead::Vector3f& vec) {
    al::HitSensor* hitSensor = nullptr;
    if (al::isCollidedWallVelocity(this))
        hitSensor = al::getCollidedWallSensor(this);

    if (!hitSensor && al::isCollidedGround(this) &&
        vec.dot(sead::Vector3f::ey) < sead::Mathf::cos(sead::Mathf::deg2rad(135))) {
...
void CatchBomb::exeMarch() {
    if (al::isFirstStep(this)) {
        f32 velLength = al::getVelocity(this).length();
        sead::Vector3f dir = rs::getPlayerHeadPos(al::getPlayerActor(this, 0)) - al::getTrans(this);
        dir.y = 0.0f;
        if (al::tryNormalizeOrZero(&dir))
            al::getVelocityPtr(this)->set(dir * velLength);
    }
    ...
    if (al::getVelocity(this).squaredLength() < 1225.0f) {
...
void CatchBomb::exeHide() {
    ...
    if (al::isGreaterEqualStep(this, 60)) {
        ...
        al::resetPosition(this, mPosition);
...
void CatchBomb::control() {
    ...
    if (al::isNearPlayerH(this, 2000.0f))
        if (sead::Mathf::abs(al::getPlayerPos(this, 0).y - al::getTrans(this).y) > 200.0f)
            _148 += 1;
        else
            _148 = 0;
    else
