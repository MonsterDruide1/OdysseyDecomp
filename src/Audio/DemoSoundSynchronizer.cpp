...
void DemoSoundSynchronizer::startSync() {
    mIsHandheld =
        mApplicationMessageReceiver->getStoredOperationMode() == nn::oe::OperationMode_Handheld;
    mIsSyncActive = true;
}
...
void DemoSoundSynchronizer::trySync(s32 delay, al::DemoSyncedEventKeeper* syncedEventKeeper) {
    if (!mIsSyncActive)
        return;
    ...
    trySyncCommon(delay, seCtrl->getCurPlayingSeStartFrame(),
                  seKeeper->tryGetCurrentStreamSamplePos(name, nullptr));
}

void DemoSoundSynchronizer::trySyncCommon(s32 currentTime, s32 delayFrames, s64 sample) {
    mIsHandheld =
        mApplicationMessageReceiver->getStoredOperationMode() == nn::oe::OperationMode_Handheld;

    if (!mAudioDirector->isPpausedSystem("システムポーズ") && sample >= 0) {
        s32 resumeDelay = s32((f32(sample) / 48000.0f) * 60.0f) + (delayFrames - currentTime);
        if (resumeDelay > 4) {
            pause();
...
void DemoSoundSynchronizer::trySync(s32 currentTime, s32 delayFrames, s64 sample) {
    if (mIsSyncActive)
        trySyncCommon(currentTime, delayFrames, sample);
}
