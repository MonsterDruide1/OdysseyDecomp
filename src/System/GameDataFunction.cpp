...
void findAreaAndChangeNextStage(GameDataHolderWriter writer, const al::LiveActor* actor,
    ...
    if (writer->getGameDataFile()->get_0xa2c() &&
...
void getLifeMaxUpItem(const al::LiveActor* player) {
    if (PlayerFunction::isPlayerDeadStatus(player))
        return;
    GameDataHolderAccessor accessor{player};
...
void recoveryPlayer(const al::LiveActor* player) {
    if (PlayerFunction::isPlayerDeadStatus(player))
        return;
    GameDataHolderAccessor accessor{player};
...
void recoveryPlayerForDebug(const al::LiveActor* player) {
    if (PlayerFunction::isPlayerDeadStatus(player))
        return;
    GameDataHolderAccessor accessor{player};
...
void recoveryPlayerMax(const al::LiveActor* player) {
    if (PlayerFunction::isPlayerDeadStatus(player))
        return;
    GameDataHolderAccessor accessor{player};
...
bool checkLatestHintSeaOfTree(GameDataHolderAccessor accessor) {
    return isSeaOfTreeStage(accessor->getGameDataFile()->getLatestHint()->stage_name.cstr());
}
...
bool checkHintSeaOfTree(GameDataHolderAccessor accessor, s32 index) {
    return isSeaOfTreeStage(accessor->getGameDataFile()->findHint(index).stage_name.cstr());
}

bool checkHintSeaOfTreeMoonRock(GameDataHolderAccessor accessor, s32 index) {
    return isSeaOfTreeStage(accessor->getGameDataFile()->findHintMoonRock(index).stage_name.cstr());
}
...
s64 getLastUpdateFileTime(const al::LayoutActor* layout, s32 file_id) {
    GameDataHolderAccessor accessor{layout};
...
const char16* tryFindShineMessage(const al::LayoutActor* layout, s32 world_id, s32 index) {
    ...
    al::StringTmp<128> label;
    al::PlacementId placement_id{info->obj_id.cstr(), nullptr, nullptr};
    rs::makeMessageLabel(&label, &placement_id, "ScenarioName");
    return al::isExistLabelInStageMessage(layout, info->stage_name.cstr(), label.cstr()) ?
               al::getStageMessageString(layout, info->stage_name.cstr(), label.cstr()) :
               u"シャイン名未設定";
}
...
const char16* tryFindShineMessage(const al::LiveActor* actor,
    ...
    al::PlacementId placement_id{info->obj_id.cstr(), nullptr, nullptr};
    rs::makeMessageLabel(&label, &placement_id, "ScenarioName");
    return al::isExistLabelInStageMessage(message_system, info->stage_name.cstr(), label.cstr()) ?
               al::getStageMessageString(message_system, info->stage_name.cstr(), label.cstr()) :
               u"シャイン名未設定";
}

s64 findShineGetTime(const al::LayoutActor* layout, s32 world_id, s32 index) {
    return getGameDataHolder(layout)->getGameDataFile()->findShine(world_id, index)->get_time;
}

bool checkMoonRockShineForShineList(const al::LayoutActor* layout, s32 world_id, s32 index) {
    return getGameDataHolder(layout)->getGameDataFile()->findShine(world_id, index)->is_moon_rock;
}
...
bool checkShineSeaOfTreeForShineList(const al::LayoutActor* layout, s32 world_id, s32 index) {
    return isSeaOfTreeStage(getGameDataHolder(layout)
                                ->getGameDataFile()
                                ->findShine(world_id, index)
                                ->stage_name.cstr());
}
