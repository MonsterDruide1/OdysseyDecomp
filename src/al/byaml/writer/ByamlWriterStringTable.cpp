...
const char* ByamlWriterStringTable::tryAdd(const char* string) { //TODO major mismatches, probably due to the inlined functions way of adding things to `list`.
    for(auto& node : mList){
        int result = strcmp(string, node);
        if(result == 0) return node;
        if(result < 0) return add(string, mList);
    }
