...
void PlayerWallActionHistory::update(const IUsePlayerCollision* collision) {
...
void PlayerWallActionHistory::recordWallJump(const IUsePlayerCollision* collision,
                                             const sead::Vector3f& position) {
    mIsJumpStored = true;
    mJumpWallPosition = position;
    mJumpWallNormal = rs::getCollidedWallNormal(collision);
}
