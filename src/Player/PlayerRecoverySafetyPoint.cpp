...
void PlayerRecoverySafetyPoint::noticeDangerousPoint(const sead::Vector3f& pos, bool adjustFlag) {
    if (rs::isPlayer2D(mActor)) {
        if (rs::isIn2DArea(mDimension)) {
            ...
            slideLastSafetyPoint(&mSafety2D.mSafetyPos, &mSafety2D.mSafetyNormal,
                                 mSafety2D.mHasSafety, snapPos, adjustFlag, mColFilter);
        }

        return;
    }

    slideLastSafetyPoint(&mSafety3D.mSafetyPos, &mSafety3D.mSafetyNormal, mSafety3D.mHasSafety, pos,
                         adjustFlag, nullptr);
    return;
}
...
bool PlayerRecoverySafetyPoint::isValid() const {
    if (isEnableRecovery()) {
        if (rs::isPlayer2D(mActor))
            return mSafety2D.mHasSafety;
        return mSafety3D.mHasSafety;
    }
    return false;
}
...
const sead::Vector3f& PlayerRecoverySafetyPoint::getSafetyPoint() const {
    ...
    if (rs::isPlayer2D(mActor))
        return mSafety2D.mSafetyPos;
    else
        return mSafety3D.mSafetyPos;
}

const sead::Vector3f& PlayerRecoverySafetyPoint::getSafetyPointGravity() const {
    if (rs::isPlayer2D(mActor))
        return mSafety2D.mGravity;
    return mSafety3D.mGravity;
}

const al::AreaObj* PlayerRecoverySafetyPoint::getSafetyPointArea() const {
    if (rs::isPlayer2D(mActor))
        return mSafety2D.mArea;
    return mSafety3D.mArea;
}
...
void PlayerRecoverySafetyPoint::startRecovery(float height) {
