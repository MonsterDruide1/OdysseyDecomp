...
void PlayerDamageKeeper::invalidate(s32 value) {
    mIsUpdateModel = true;
    mCurrentHealth = sead::Mathi::clampMin(mCurrentHealth, value);
}

void PlayerDamageKeeper::invalidateIncludePush(s32 value) {
    invalidate(value);
    mPush = value;
}
...
void PlayerDamageKeeper::update(IPlayerModelChanger* modelChanger, bool unk, bool isPreventDamage) {
    if (isPreventDamage)
        updatePreventDamage();
    updateKidsModeLifeRecovery(unk);
...
void PlayerDamageKeeper::updateKidsModeLifeRecovery(bool unk) {
    ...
    mKidsModeTimerForAutoHeal = unk ? al::converge(mKidsModeTimerForAutoHeal, 120, 1) : 0;
