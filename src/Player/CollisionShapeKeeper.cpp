...
void CollisionShapeKeeper::createShapeDiskSupportGround(const char* name, f32 a1, const sead::Vector3f& a2,
                                                        const sead::Vector3f& a3, f32 a4,
...
void CollisionShapeKeeper::createShapeDiskIgnoreGround(const char* name, f32 a1, const sead::Vector3f& a2,
                                                       const sead::Vector3f& a3, f32 a4) {
    auto* info = new CollisionShapeInfoDisk(name, a1, a2, a3, a4);
    info->setIgnoreGround();
    mCollisionShape.pushBack(info);
...
void CollisionShapeKeeper::updateShape() {
    ...
    for (s32 i = 0; i < numShapes; i++) {
        ...
        if(al::isNearZero(centerDist, 0.001f)) {
            ...
            sead::Vector3f test = radius*offsetDirection + center;
            sead::Vector3f test2 = mBoundingCenter - offsetDirection * mBoundingRadius;
            sead::Vector3f diff = test - test2;
            mBoundingCenter = test2 + diff * 0.5f;
            mBoundingRadius = diff.length() * 0.5f;
        }
    }

    mCheckStepRange = mBoundingRadius;

    f32 maxRange = mCheckStepRange;
    for (s32 i = 0; i < numShapes; i++) {
        f32 checkRange = mCollisionShape[i]->getCheckStepRange();
        maxRange = maxRange < checkRange ? maxRange : checkRange;
        mCheckStepRange = maxRange;
    }
}
