...
void PlayerActionVelocityControl::calcTrample(f32 speedDown) {
    al::verticalizeVec(&mVelocityUp, al::getGravity(mActor), mVelocityUp);
    mVelocityUp += al::getGravity(mActor) * speedDown;
}
void PlayerActionVelocityControl::calcSnap(const sead::Vector3f& snapDirection, f32 snapDistance) {
    // requires to be this explicit to match. Both += and vector operations mismatch.
    mVelocityUp.x = (snapDirection.x * snapDistance) + mVelocityUp.x;
    mVelocityUp.y = (snapDirection.y * snapDistance) + mVelocityUp.y;
    mVelocityUp.z = (snapDirection.z * snapDistance) + mVelocityUp.z;
}
