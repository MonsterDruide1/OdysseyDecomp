...
def common_const_reference(c, path):
    for line in c.splitlines():
        if "& " in line and line[line.find("& ") - 1] != "&" and line[line.find("& ") - 1] != " " and "CLASS&" not in line:
            if ("const" not in line or line.find("& ") < line.find("const ")) and ("for" not in line or " : " not in line) and ("operator->" not in line) and ("AudioDirectorInitInfo" not in line) and ("sead::IDelegate1<CollisionParts*>" not in line):
