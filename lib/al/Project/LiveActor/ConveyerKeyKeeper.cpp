...
void ConveyerKeyKeeper::calcPosAndQuat(sead::Vector3f* pos, sead::Quatf* quat, s32* index,
    ...
    if (keyIndex < 1) {
        ...
        v.set(key.moveDistanceVertical);
        q.set(key.quat);
        value = 0.0f;
    } else {
        ...
        f32 t = key.totalMoveDistance - prevKey.totalMoveDistance;
        if (isNearZero(t))
            t = 0.0f;
        else
            t = (coord - getConveyerKey(keyIndex - 1).totalMoveDistance) / t;

        f32 ease = easeByType(t, getConveyerKey(keyIndex - 1).interpolateType);
        lerpVec(&v, prevKeyVec, keyVec, ease);
        value = lerpValue(getConveyerKey(keyIndex - 1).moveDistance,
