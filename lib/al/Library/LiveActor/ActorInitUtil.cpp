...
__attribute__((always_inline)) void initActorModel(LiveActor* actor, const ActorInitInfo& initInfo,
    ...
    if (modelLodCtrl)
        initInfo.getActorSceneInfo().graphicsSystemInfo->modelLodAllCtrl->registerLodCtrl(
            modelLodCtrl);

    MaterialCategoryKeeper* materialCategoryKeeper =
        initInfo.getActorSceneInfo().graphicsSystemInfo->materialCategoryKeeper;
...
void initActorImpl(LiveActor* actor, const ActorInitInfo& initInfo,
    ...
    ActorResource* actorResource =
        findOrCreateActorResource(initInfo.getActorResourceHolder(), path.cstr(), suffix);
...
void makeMapPartsModelName(sead::BufferedSafeString* modelName, sead::BufferedSafeString* path,
                           const ActorInitInfo& initInfo) {
    makeMapPartsModelName(modelName, path, initInfo.getPlacementInfo());
}
...
PartsEffectGroup* createPartsEffectGroup(LiveActor* actor, const ActorInitInfo& initInfo,
    ...
    group->init(maxEffects, initInfo.getEffectSystemInfo(), name, getTransPtr(actor),
