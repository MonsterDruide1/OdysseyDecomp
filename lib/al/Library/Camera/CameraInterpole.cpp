        ...
        if ((mPrevTicket && !alCameraPoserFunction::isInvalidCollider(mPrevTicket->getPoser())) ||
            ...
            if (tryNormalizeOrZero(&a3)) {
                ...
                for (u64 v15 = 0; v15 != 5; v15++) {
                    ...
                    rotateVectorDegree(&v53, v53, a3, v54[v15].x);
                    rotateVectorDegree(&v53, v53, a1, v54[v15].y);
...
void rotate(sead::Vector3f* result, const sead::Vector3f& vec, const sead::Quatf& quat) {
    f32 v19 = (vec.z * quat.y) - (vec.y * quat.z) + (vec.x * quat.w);
    f32 v20 = (vec.x * quat.z) - (vec.z * quat.x) + (vec.y * quat.w);
    f32 v22 = (vec.y * quat.x) - (vec.x * quat.y) + (vec.z * quat.w);
    f32 v23 = -(vec.x * quat.x) - (vec.y * quat.y) - (vec.z * quat.z);
    result->x = (quat.y * v22) - (quat.z * v20) + (quat.w * v19) - (quat.x * v23);
    result->y = (quat.z * v19) - (quat.x * v22) + (quat.w * v20) - (quat.y * v23);
    result->z = (quat.x * v20) - (quat.y * v19) + (quat.w * v22) - (quat.z * v23);
}
