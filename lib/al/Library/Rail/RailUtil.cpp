...
bool moveRailTurn(IUseRail* railHolder, f32 speed, f32 coord) {
    if (speed < 0.0f)
        reverseRail(railHolder);

    bool isReversed = false;
    f32 absSpeed = sead::Mathf::abs(speed);
    moveRail(railHolder, absSpeed);
    if (coord <= 0.0f) {
        if (isRailReachedGoal(railHolder))
            isReversed = true;
    } else if (!isLoopRail(railHolder)) {
        if (!isRailGoingToEnd(railHolder)) {
            if (coord >= getRailCoord(railHolder))
                isReversed = true;
        } else {
            if (getRailTotalLength(railHolder) - coord <= getRailCoord(railHolder))
                isReversed = true;
        }
    }
...
bool isRailReachedNearGoal(const IUseRail* railHolder, f32 coord) {
    if (isLoopRail(railHolder))
        return false;

    if (!isRailGoingToEnd(railHolder)) {
        if (getRailCoord(railHolder) <= coord)
            return true;
    } else {
        if (getRailTotalLength(railHolder) - coord <= getRailCoord(railHolder))
            return true;
...
s32 calcCoordNearestRailPointNo(const IUseRail* railHolder) {
    ...
    if (isLoopRail(railHolder)) {
        pointNo = 0;
        if (index != getRailPointNum(railHolder))
            pointNo = index;
    }
    return pointNo;
}
...
s32 getNextRailPointNo(const IUseRail* railHolder) {
    ...
    s32 lastPoint = sead::Mathi::clampMax(newIndex, railPointNum - 1);

    return (0 > newIndex) ? 0 : lastPoint;
}
...
bool isExistRail(const IUseRail* railHolder) {
    return getRailRider(railHolder);
}
...
bool isRailReachedNearGoal(const IUseRail* railHolder, f32 coord1, f32 coord2) {
    if (isLoopRail(railHolder))
        return false;

    if (!isRailGoingToEnd(railHolder)) {
        if (getRailCoord(railHolder) <= coord1)
            return true;
    } else {
        if (getRailTotalLength(railHolder) - coord2 <= getRailCoord(railHolder))
            return true;
    }
    return false;
}
...
bool isRailPlusDir(const IUseRail* railHolder, const sead::Vector3f& dir) {
    return 0.0f <= dir.dot(getRailDir(railHolder));
}
...
s32 calcForwardRailPointNo(const IUseRail* railHolder, s32 index) {
    ...
    if (isRailGoingToEnd(railHolder)) {
        if (pointNum - 1 == index)
            return isLoopRail(railHolder) - 1;
...
void calcRailClippingInfo(sead::Vector3f* pos, f32* distance, const IUseRail* railHolder, f32 step,
    ...
    *distance = sead::Mathf::max((lastRailPos - *pos).length(), *distance) + offset;
}
...
void calcAndSyncMoveFitRailDir(IUseRail* railHolder, sead::Vector3f* out, f32 step,
    ...
    sead::Vector3f verticalDir;
    verticalizeVec(&verticalDir, vertical, railDir);
    tryNormalizeOrZero(&verticalDir);

    sead::Vector3f railPos = railRider->getPosition() - pos;
    sead::Vector3f verticalPos;
    verticalizeVec(&verticalPos, vertical, railPos);

    f32 scalar;
    separateScalarAndDirection(&scalar, &verticalPos, verticalPos);

    lerpVec(out, verticalDir, verticalPos, sead::Mathf::clamp(scalar / step, 0.0f, 1.0f));
    tryNormalizeOrZero(out);
}

void calcAndSyncMoveFitRailDir(LiveActor* actor, sead::Vector3f* out, f32 step, bool reverse) {
    calcAndSyncMoveFitRailDir(actor, out, step, getTrans(actor), getGravity(actor), reverse);
}
...
void placementRailDivideForcePoint(const IUseRail* railHolder, f32 segmentLength,
    ...
    for (s32 i = 0; i < railNum; i++) {
        f32 length = getRail(railHolder)->getPartLength(i);
        s32 limit = (s32)(length / segmentLength) + 1;
        for (s32 e = 0; e < limit; e++) {
