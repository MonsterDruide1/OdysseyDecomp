...
bool moveRailTurn(IUseRail* railHolder, f32 speed, f32 goalCoord) {
    ...
    if (goalCoord <= 0.0f) {
        ...
        isReversed = isRailReachedNearGoal(railHolder, goalCoord);
    }
...
bool isRailReachedNearGoal(const IUseRail* railHolder, f32 goalCoord) {
    ...
    if (isRailGoingToEnd(railHolder)) {
        if (getRailTotalLength(railHolder) - goalCoord <= getRailCoord(railHolder))
            return true;
    } else {
        if (getRailCoord(railHolder) <= goalCoord)
            return true;
    }
    return false;
}
