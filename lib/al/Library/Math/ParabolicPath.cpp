...
void ParabolicPath::initFromUpVector(const sead::Vector3f& start, const sead::Vector3f& end,
    ...
    f32 scalar;
    sead::Vector3f upDir;
    al::separateScalarAndDirection(&scalar, &upDir, up);
...
void ParabolicPath::initFromUpVector(const sead::Vector3f& start, const sead::Vector3f& end,
    ...
    al::separateScalarAndDirection(&mHorizontalDistance, &mHorizontalDirection,
                                   mHorizontalDirection);
    al::calcParabolicFunctionParam(&mGravity, &mInitialVelY, maxHeight, verticalDistance);
    mStart.set(start);
}
...
void ParabolicPath::initFromMaxHeight(const sead::Vector3f& start, const sead::Vector3f& end,
    ...
    sead::Vector3f up;
    f32 scalar;
    al::separateScalarAndDirection(&scalar, &up, projectedEnd - end);
...
void ParabolicPath::calcPositionEaseOutH(sead::Vector3f* pos, f32 prog) const {
    calcPositionHV(pos, al::easeOut(prog), prog);
}
