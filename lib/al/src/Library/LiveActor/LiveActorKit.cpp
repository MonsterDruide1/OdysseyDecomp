...
LiveActorKit::LiveActorKit(int maxActors, int maxPlayers) : mMaxActors(maxActors) {
...
LiveActorKit::~LiveActorKit() {
    ...
    if (mDynamicDrawActorGroup) {
        for (int i = 0; i < mDynamicDrawActorGroup->getActorCount(); i++) {
...
void LiveActorKit::init(int maxCameras) {
    ...
    mExecutor1 = new ExecuteAsyncExecutorUpdate(mExecuteDirector, "ビュー更新(コア1)", 1);
    mExecutor2 = new ExecuteAsyncExecutorUpdate(mExecuteDirector, "ビュー更新(コア2)", 2);
...
void LiveActorKit::initSwitchAreaDirector(int val1, int val2) {
    mSwitchAreaDirector =
...
void LiveActorKit::endInit() {
    ...
    for (int i = 0; i < mLiveActorGroupAllActors->getActorCount(); i++) {
...
void LiveActorKit::updateGraphics() {
    mExecutor1->executeAsync();
    mExecutor2->executeAsync();
